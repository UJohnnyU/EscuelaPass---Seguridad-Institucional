...
> **Nota de maquetación (borrador maestro TDG-2).** Este archivo Markdown consolida el **texto principal** del trabajo de grado y los **anexos 00–10** en un solo flujo para volcado a Word (Times New Roman 12 pt, márgenes y estilo APA 7 según manual APIT). Las figuras incluyen *placeholders*; los *prompts* para generación de imágenes se documentan por separado en `docs/tdg/prompts-imagenes-ia.md`.
...
This thesis develops Escuela Pass, a web application proposed by AlfaNetworks with a modular architecture based on NestJS (NestJS Team, n.d.) and PostgreSQL (PostgreSQL Global Development Group, n.d.), a React client (React Team, n.d.) packaged with Vite (Vite Team, n.d.), access control via QR codes (Quick Response) and NFC (Near Field Communication), and a family pick-up system supported by GPS (Global Positioning System). The platform integrates identity, access control, school management, institutional communication, and a payment system with human verification of receipts—without an automated bank gateway within the documented scope. The general objective approved in the Thesis Statement (FTG) is fully addressed, and the research question guides the argumentation throughout the document.

In methodological summary, an iterative and incremental process was applied based on the Scrum framework (Schwaber & Sutherland, 2020), with five phases aligned to the five approved specific objectives (requirements analysis; design; service implementation; client development; and validation). The results show a product consistent with the agreed-upon functional and non-functional requirements, with the following explicit limitations: storage of session credentials in the browser, test coverage biased towards end-to-end testing, and performance and usability metrics subject to subsequent measurement with pilot users using the System Usability Scale (SUS) (Brooke, 1996). It is concluded that the proposal contributes to optimizing operational processes and implements privacy and security controls appropriate for responsible deployment in the Mexican context, without replacing institutional policies or external legal counsel.
...
*Generar en Microsoft Word mediante estilos de título y referencias cruzadas. La siguiente lista orienta la jerarquía del borrador maestro (en este `.md`, resumen, *abstract*, índice y lista de figuras/tablas van antes del capítulo 1).*

Resumen · Abstract · Tabla de contenido · Lista de figuras y tablas *(maquetar según instructivo; si Word exige lista de ilustraciones vacía, usar el mismo criterio institucional)*

1. Introducción
2. Presentación del trabajo (planteamiento del problema; justificación; objetivos)
...
Anexos (texto completo incorporado al final de este archivo maestro)
...
| Figura 3 | Diagrama orientativo del orden del documento y sus anexos (sección 3.6). |
| Figura 4 | Cronograma Gantt del proyecto (24/02/2026 – 21/06/2026) (sección 3.7). |
| Figura 5 | Arquitectura general por capas del sistema Escuela Pass (sección 4.1). |
| Figura 6 | Modelo entidad–relación resumido por dominios funcionales (sección 4.1). |
| Figura 7 | Diagrama UML de casos de uso por actor institucional (sección 5.1). |
| Figura 8 | Diagrama UML de secuencia del circuito de recogida familiar (sección 5.3). |
| Figura 9 | Diagrama UML de secuencia del escaneo de credencial QR/NFC (sección 5.3). |
| Figura 10 | Licencia del proyecto (imagen pendiente de aporte por el autor) (sección 9). |

| Tabla | Contenido |
| --- | --- |
| Tabla 1 | Problemática: causas y consecuencias (sección 2.1.3, alineada a la FTG) |
| Tabla 2 | Fases metodológicas y resultados esperados (capítulo 3 — diseño metodológico) |
| Tabla 3 | Limitaciones de antecedentes (capítulo 4 — marco referencial) |
| Tabla 4 | Inclusiones del alcance (capítulo 4 — marco referencial) |
| Tabla 5 | Exclusiones del alcance (capítulo 4 — marco referencial) |
| Tabla 6 | Actores y responsabilidades macroscópicas (capítulo 5 — desarrollo) |
...
Tras los elementos preliminares (resumen, *abstract*, tabla de contenido e índices auxiliares), la introducción contextualiza el trabajo; el **capítulo 2** integra planteamiento del problema, justificación por impactos y objetivos; el **capítulo 3** desarrolla el diseño metodológico apoyado en el marco Scrum (Schwaber & Sutherland, 2020); el **capítulo 4** presenta el marco referencial y el alcance; el **capítulo 5** detalla el desarrollo por objetivo específico; el **capítulo 6** consolida resultados y discusión frente a la pregunta de investigación. Cierran las conclusiones (**7**), las recomendaciones y trabajos futuros (**8**), la licencia del proyecto (**9**), las referencias, el glosario de acrónimos y los anexos numerados.
...
#### 2.1.2. Identificación del PIN (Problema, Idea o Necesidad)
...
#### 2.1.4. Formulación del problema — pregunta de investigación
...
#### 2.1.5. Alcances excluidos o diferidos (síntesis)
...
### 3.2. Fuentes de información y técnicas de verificación
...
### 3.3. Fases del trabajo (por objetivo específico)
...
### 3.4. Tecnologías y herramientas (visión sintética)
...
### 3.5. Decisiones técnicas adoptadas en la ejecución que no figuraban punto por punto en la propuesta inicial
...
Cada una de estas decisiones se retoma en los capítulos **5 y 6** (desarrollo y resultados) con la misma tríada cuando corresponde, y se concentra de forma tabular en el anexo de arquitectura.

### 3.6. Estructura del documento (representación gráfica)
...
*[Figura 5. Arquitectura general por capas del sistema Escuela Pass: cliente web, servicio backend con NestJS, persistencia en PostgreSQL e integraciones externas opcionales (FCM, Mapbox, SMTP). Recomendado: Lucidchart con asistencia de Lucid AI; el *prompt* exacto para generación de imagen se documenta en el archivo de apoyo del repositorio bajo `docs/tdg/` (véase índice de figuras y documentación de *prompts* asociada).]*
...
En los últimos cinco años la literatura y la industria han propuesto plataformas de gestión escolar, soluciones de control de acceso desacopladas del frente académico y herramientas de mensajería informal entre escuela y familia. Investigaciones sectoriales sobre educación escolar digital y transformación post‑pandemia convergen en señalar brechas y condiciones para adopción responsable de aplicaciones (Martínez et al., 2025; UNESCO, 2024). La documentación técnica de las pilas empleadas (NestJS Team, s. f.; PostgreSQL Global Development Group, s. f.; React Team, s. f.) orienta las decisiones de implementación junto con el planteamiento sobre arquitecturas de red de Fielding (2000) y la visión de sistemas de información organizacionales de Laudon y Laudon (2018).

Lo que distingue a Escuela Pass de los antecedentes consultados es la **orquestación** dentro de un mismo producto de identidad, acceso físico, circuito familiar, operación académico‑administrativa, comunicación institucional y cartera con revisión humana, con trazabilidad entre documentación y repositorio verificable en la revisión académica del trabajo.

### 4.4. Limitaciones de los antecedentes y de la literatura revisada
...
### 5.1. Análisis de requerimientos (objetivo específico 1)
...
### 5.2. Diseño del sistema, datos e interfaz (objetivo específico 2)
...
### 5.3. Implementación del backend (objetivo específico 3)
...
### 5.4. Desarrollo del frontend (objetivo específico 4)
...
Las extensiones de producto efectivamente ejecutadas —operación multi‑institución, dominios de visitas, reuniones, anotaciones, privacidad y auditoría— constituyen evoluciones que amplían el alcance entregado sin modificar los objetivos específicos aprobados, presentadas en el anexo de arquitectura con la misma tríada motivo–problema–beneficio utilizada para el resto de decisiones técnicas documentadas.

### 5.5. Validación y cierre técnico (objetivo específico 5)

Se ejecutaron baterías de pruebas extremo a extremo del servicio contra una base de datos PostgreSQL real, automatizadas en integración continua sobre la infraestructura del repositorio. Las pruebas de humo combinadas verifican el arranque del servicio y los flujos críticos de autenticación, control de accesos, circuito, asistencia, calificaciones y finanzas. La cobertura automatizada se concentra en pruebas extremo a extremo, y el plan de pruebas deja formulada como mejora la ampliación de pruebas unitarias en dominios sensibles.

La medición fina de tiempos de respuesta y la evaluación de usabilidad con usuarios piloto quedan **planificadas** en el anexo de pruebas como **plan de medición** —no como medición ejecutada— dado que el horizonte del trabajo de grado no alcanzó a registrar muestras con significación estadística. Esa delimitación metodológica se mantiene explícita en las secciones correspondientes.

---

## 6. RESULTADOS Y DISCUSIÓN

### 6.1. Síntesis cualitativa frente a la pregunta de investigación

La pregunta de investigación formulada en la sección **2.1.4** orienta la discusión que sigue. Esa discusión cualitativa sitúa los hallazgos en tres ejes: factibilidad técnica de integrar seguridad física, tratamiento de datos y administración escolar en un mismo producto; trazabilidad documental y separación entre automatización de software y decisión institucional; y límites respecto del cumplimiento jurídico efectivo, que depende del responsable del tratamiento y de la asesoría jurídica.

### 6.2. Cumplimiento por objetivo específico
...
**Validación.** Existe evidencia de pruebas extremo a extremo automatizadas contra una base de datos real, con integración continua reproducible en la plataforma del repositorio. Las mediciones de desempeño y usabilidad con muestras institucionales reales quedaron formalmente planificadas como plan de medición, en concordancia con la delimitación ya expuesta en este capítulo.

### 6.3. Límites de la evidencia de medición
...
### 6.4. Papel de las decisiones técnicas evolutivas en el resultado

Las decisiones de despliegue sobre plataforma como servicio, la elección del proveedor de mapas con fallback de cálculo, la concreción de bibliotecas para identificación QR y la confirmación operativa del padre o tutor como conductor del circuito mejoraron la probabilidad de demostrar el producto en condiciones realistas, sin alterar el núcleo de la pregunta de investigación. En el anexo de arquitectura, la tabla de desviaciones explicita el motivo de cada decisión, el problema abordado y el efecto observado, de modo que el razonamiento técnico quede trazable.
...
## 9. LICENCIA DEL PROYECTO ESCUELA PASS
...
En esta sección se incorporan los anexos numerados **00 a 10** del trabajo de grado en el orden académico establecido. Cada anexo se presenta a continuación de forma continua para facilitar la revisión y el volcado a un documento Word único. El índice del cuerpo principal se genera al maquetar el documento final aplicando estilos de título en el procesador de textos.
...
La presente sección consolida las desviaciones técnicas adoptadas durante la ejecución frente al texto inicial de la propuesta formal. Cada decisión se documenta siguiendo la **tríada motivo–problema–beneficio** recomendada por el asesor, y se enlaza con el cuerpo del trabajo de grado (capítulo **3** — diseño metodológico, subsección 3.5; capítulo **5** — desarrollo, secciones 5.1 a 5.5; capítulo **6** — resultados y discusión, sección 6.4). Estas desviaciones **no alteran la pregunta de investigación ni los cinco objetivos específicos aprobados**: refinan medios técnicos del alcance ya trazado.
...
- **Académico:** el **Anexo 03** (§12.1) puede incorporar un párrafo de *trade-off* sobre la coexistencia de migraciones, *runtime* y SQL v4 frente a las restricciones de despliegue en cada entorno.  
...
- **Académico:** una matriz **operación OpenAPI ↔ caso E2E ↔ RF** (en **Anexo 10** o **Anexo 00**, §5 si existe) explicita la correspondencia entre contrato publicado, pruebas ejecutadas y requerimientos.  
...
| Anexo | Contenido vinculado a este documento |
| --- | --- |
| 00 | Matriz RF/RNF—implementación—rutas API; **§12.8** declaración explícita de cobertura. |
...
| Anexo | Contenido vinculado a este documento |
| --- | --- |
| 00 | Matriz RF/RNF—pantallas—evidencia; coherencia con **§15**; delimitación de cobertura (**§16.5**). |
