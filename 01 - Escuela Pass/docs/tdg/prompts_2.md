# Figuras TDG‑2 Escuela Pass — Guía v2 (`prompts_2`)

**Versión.** Reemplazo ampliado de [prompts-imagenes-ia.md](prompts-imagenes-ia.md) tras auditoría Código ↔ TDG v2 (mayo 2026). Corrige referencias § del maestro, amplía secuencias Anexo 05, pantallas 06/09 y plantillas Anexo 10. **No modifica** el borrador maestro v2.
...
### Verificación integrada (repo al cierre documental)

| Métrica | Maestro Tabla 7 | Repo | Estado |
| --- | --- | --- | --- |
| Entidades TypeORM | 49 | 49 `src/database/entities/*.entity.ts` | OK |
| Controladores `@Controller` | 32 | 32 | OK |
| Operaciones HTTP `api/v1` | 241 | 241 (decoradores) | OK |
| Migraciones versionadas | 39 | 39 | OK |
| Casos E2E Jest | 47 | 47 `it()` | OK |
| Páginas React | 26 | 26 en `frontend/src/pages` | OK |
| Módulos dominio | 37 carpetas | **32** imports en `AppModule` + submódulos (`meetings` vía `DashboardModule`) | OK con matiz |
| Casos de uso Anexo 05 §2 | 67 filas | 67/67 con evidencia código (ver § Matriz UC) | OK |

**Principio de trazabilidad:** código → TDG (Anexos 00–10) → prompts de esta guía. RF1–RF8 según Anexo 00/01 L832–839 (no inventar RF9).
...
| Fig. 3 | § 2.6 | A + B | § Figura 3 |
| Fig. 4 | § 2.7 | A + B | § Figura 4 |
| Fig. M1 | § 3.1 | A | § Figura M1 |
| Fig. 5 | § 3.1 | **A** (Mermaid) + B | § Figura 5 |
| Fig. 6 | § 3.1 | **C** (DBML macro) + B/C | § Figura 6 |
...
| Fig. 10 | § 8 (cap. Licencia) | Asset legal | § Figura 10 |
...
## Figura 3 — Orden del documento y anexos (§ 2.6)
...
## Figura M1 — Arquitectura lógica por capas (§ 3.1)
...
**Docente, Padre o tutor, Alumno** — idem (13 + 16 + 9 filas §2.3–2.5).
...
**Nota trazabilidad código (RF4).** `ImportExportPage` en `App.tsx` admite rol **DOCENTE** además de `ADMIN`/`ADMINISTRATIVO`; el Anexo 05 §2.2 no enumera importación para docente — si se captura pantalla, documentar como *alcance UI ampliado* o restringir captura a personal administrativo según criterio del asesor.

**Pantallas:** `SchoolRosterPage`, `SchoolsAdminPage`, `InstitutionPage` (`/app/institucion`), `ImportExportPage`, `VisitasPage`, `ReunionesPage`, `ScheduleHubPage` / `StaffScheduleBrowsePage`.
...
**Pantallas / API:** `ComunicacionPage` (`/app/modulos/comunicacion`, acceso desde `AppHomePage`); `notices.controller`, `notifications.controller` (`admin-reports` para SLA §2.2).
...
**Pie maestro.** *Diagrama de secuencia del escaneo QR de credencial con regla antiduplicación y marcado automático de asistencia* (Anexo 05 §3.3; paralelo M3 / Fig. 9 cuerpo §4.3).

**Lifelines (izquierda → derecha):**

1. *Personal portería / Docente* (actor)  
2. `:EscanerAccesoPage` (boundary SPA)  
3. `:AccessController` / `:AccessService` (controlador NestJS)  
...
| # | De → A | Mensaje | Notas |
| --- | --- | --- | --- |
| 1 | Actor → SPA | Abrir escáner; leer QR/NFC | `html5-qrcode` / Web NFC |
| 2 | SPA → API | `POST api/v1/access-events/scan` + Bearer JWT | body: token/uid credencial |
| 3 | API → DB | Buscar `access_credentials`; validar vigencia y rol | |
| 4 | API → DB | Consultar último `access_events` misma credencial | ventana **~10 s** antiduplicación |
| 5a | API → SPA | **409** o mensaje controlado si duplicado | fragmento **alt** |
| 5b | API → DB | Insertar `access_events` | rama **else** |
| 6 | API → DB | Actualizar `attendance_records` y/o `class_attendance_records` | según contexto sesión |
| 7 | API → SPA | **201** + payload (nombre, grupo, método lectura) | |

**Fragmentos UML:** `alt` [duplicado reciente] / [lectura válida]. Nota al pie: variable opcional `ATTENDANCE_ENTRY_GRACE_MINUTES` para tardanza en entrada.

#### Prompt IA (Fig. 9 / escaneo)

```text
Diagrama de secuencia UML 2.5.1 Escuela Pass escaneo QR/NFC. Actores: Personal porteria o Docente. Lifelines: EscanerAccesoPage, AccessController, PostgreSQL. Flujo: POST access-events/scan con JWT; validar credencial; alt duplicado en ventana 10 segundos retorna 409; else insertar access_events y marcar asistencia diaria o por sesion; respuesta 201. Sin pasarela. Fondo blanco. Pie: Figura 9 secuencia escaneo QR/NFC Anexo 05.
```

**Criterios de aceptación.** Endpoint y códigos HTTP alineados al maestro §4.3; coherente `access.service.ts` y E2E scan.
...
**Pie maestro.** *Diagrama de secuencia del circuito de recogida desde el cliente del padre o tutor* (Anexo 05 §3.1; paralelo M4 / Fig. 8 cuerpo §4.3).

**Lifelines:**

1. *Padre o tutor* — `:CircuitPadrePage`  
2. *Docente / Personal administrativo* — `:CircuitTodayPage` / `:CircuitDetailPage`  
3. `:CircuitController` / `:CircuitService`  
4. `:PostgreSQL`  
5. *opt* `:FCM` / `:NotificationsService`

**Mensajes numerados:**

| # | De → A | Mensaje | Regla |
| --- | --- | --- | --- |
| 1 | Padre → API | `POST api/v1/circuit-requests` (hijo, día) | 1 activa/estudiante/día |
| 2 | API → DB | Validar vínculo `student_parents`, asistencia, circuito habilitado | |
| 3 | API → Personal | Notificación nueva solicitud | opt FCM |
| 4 | Padre → API | `PATCH` ubicación GPS | Haversine en servidor |
| 5 | Personal → API | Autorizar salida / avanzar estado | estados dominio |
| 6 | Padre → API | Confirmar entrega | cierre familiar |
| 7 | API → DB | Estado `ENTREGADO` o `CANCELADO` | |

**Fragmentos:** `opt` notificación FCM; `opt` cancelación antes de entrega. **Prohibido:** actor “conductor escolar” o flota institucional.

#### Prompt IA (Fig. 8 / circuito)

```text
Secuencia UML circuito recogida familiar Escuela Pass. Operador principal: Padre o tutor en CircuitPadrePage (NO conductor escolar). Lifelines: CircuitPadrePage, CircuitTodayPage, CircuitService, PostgreSQL, FCM opcional. POST crear circuit-requests; PATCH GPS; personal autoriza estados; padre confirma entrega; opt cancelar. Nota: una solicitud activa por estudiante por dia. Pie Figura 8 Anexo 05.
```

**Criterios de aceptación.** Coherente con `circuit_requests` y `circuit.service.ts` §4.3–4.4.
...
**Composición modular (auditoría).** `AppModule` importa **32** módulos de dominio en raíz; existen **37** carpetas bajo `src/modules/`. Submódulos no raíz: `meetings` (importado por `DashboardModule`), utilerías `fcm`, `events-core`, `academic-notifications`, `pickup-requests` (auxiliar/legacy — no contar como RF independiente salvo nota al pie).
...
#### draw.io — pago con verificación humana (`A05-SEQ-3`, Anexo 05 Fig. 4)

**Pie maestro.** *Secuencia de pago con verificación humana de comprobante, sin pasarela bancaria automática.*

**Lifelines:** *Padre/Tutor* → `:FinanzasPage` → `:PaymentsController` / `:FilesController` → `:PostgreSQL` → *Personal administrativo* → `:FinanzasPage` (staff).

| # | Mensaje | Detalle código |
| --- | --- | --- |
| 1 | Padre sube archivo comprobante | `POST` uploads/files privados |
| 2 | API registra pago/deuda pendiente verificación | `payments.service` |
| 3 | Administrativo lista pendientes | `GET` payments/debts |
| 4 | **alt** verificar → actualiza saldo/deuda | sin Stripe/MercadoPago |
| 5 | **alt** rechazar → motivo al padre | |
| 6 | **opt** notificación FCM/email | |

**Nota visible en diagrama:** *Sin pasarela bancaria automática (RF6, maestro L498).*

#### Prompt IA (secuencia pago)

```text
Secuencia UML pago Escuela Pass sin pasarela. Padre sube comprobante en FinanzasPage; API payments y files privados; PostgreSQL; Personal administrativo verifica o rechaza comprobante; opt notificacion. Destacar verificacion humana obligatoria. Pie Anexo 05 Figura 4 pago verificacion humana.
```

#### draw.io — cierre de actividad académica (`A05-SEQ-4`, Anexo 05 Fig. 5)

**Pie maestro.** *Secuencia de cierre de actividad académica con validación de periodo.*

**Lifelines:** *Docente* → `:CalificacionesDocentePage` → `:ActivitiesController` → `:AcademicPeriodsService` → `:PostgreSQL`.

| # | Mensaje | Detalle |
| --- | --- | --- |
| 1 | Docente solicita cerrar actividad | |
| 2 | API valida periodo académico abierto y reglas | `academic-periods` |
| 3 | **alt** periodo cerrado → 4xx | |
| 4 | **else** marcar actividad cerrada (inmutable) | audit trail |
| 5 | **opt** Docente reabre con justificación | elipse RF5 §2.3 |

#### Prompt IA (secuencia cierre actividad)

```text
Secuencia UML cierre actividad Escuela Pass. Docente en CalificacionesDocentePage; ActivitiesController; validar AcademicPeriods; alt error 4xx si periodo cerrado; else cerrar actividad inmutable en PostgreSQL; opt reabrir con justificacion auditada. Pie Anexo 05 Figura 5 cierre actividad.
```

### Diagrama de clases (`A05-CLS-1`)

**Pie.** *Diagrama de clases de contexto NestJS y persistencia TypeORM* (Anexo 05 §4 — renombrar en Word como **A05-CLS-1** para evitar colisión con «Figura 3» del anexo).

**draw.io — torre por dominio (ejemplo circuito):**

| Capa | Clases / artefactos |
| --- | --- |
| Controller | `CircuitController` |
| Service | `CircuitService` (reglas 1/día, GPS Haversine) |
| Repository | TypeORM repos `CircuitRequestEntity`, … |
| Entity | `circuit_requests`, `vehicles`, `student_departure_consents` |

Repetir patrón para `AccessController`/`access-events` y `PaymentsController`/`payments`.

#### Prompt IA (clases)

```text
Diagrama de clases UML simplificado NestJS Escuela Pass. Paquete circuit: CircuitController, CircuitService, CircuitRequestEntity tabla circuit_requests. Relaciones: Controller usa Service, Service usa Repository TypeORM, Repository persiste Entity. Anotar reglas de negocio en notas: 1 circuito activo por estudiante por dia. Mismo patron para access-events y payments. Pie Anexo 05 diagrama clases contexto.
```

---

## Anexo 06 — Prototipos UI (8 figuras + rutas ampliadas)

| ID | Pie maestro (resumen) | Herramienta | Ruta / componente | Roles (`RoleGate` / nav) |
| --- | --- | --- | --- | --- |
| A06-1 | Login + recuperar contraseña | Captura o Figma | `/login`, `/recuperar-contrasena` | público |
| A06-2 | Tablero ADMIN plataforma | Captura | `/app`, `/app/escuelas` | `ADMIN` |
| A06-3 | Tablero administrativo | Captura | `/app`, `/app/gestion-escolar`, `/app/modulos/administracion` | `ADMIN`, `ADMINISTRATIVO` |
| A06-4 | Tablero docente | Captura | `/app/modulos/calificaciones-docente`, `/app/acceso/escaner` | `DOCENTE` |
| A06-5 | Vista padre/tutor | Captura móvil | `/app/circuito`, `/app/modulos/finanzas` | `PADRE` |
| A06-6 | Vista alumno | Captura | `/app/modulos/mis-calificaciones`, `/app/horario` | `ALUMNO` |
| A06-7 | Escáner QR/NFC | Captura móvil | `/app/acceso/escaner` | `ADMIN`, `ADMINISTRATIVO`, `DOCENTE` |
| A06-8 | Mapa circuito Mapbox | Captura móvil | `CircuitPadrePage` / detalle — datos sintéticos | `PADRE` |
| A06-9* | Comunicación / avisos | Captura | `/app/modulos/comunicacion` (`ComunicacionPage`) | todos autenticados; **sin ítem en `navConfig`** — acceso desde `AppHomePage` |
| A06-10* | Institución / parámetros | Captura | `/app/institucion` (`InstitutionPage`) | todos autenticados (`nav`: Institución) |

\*Filas ampliadas v2 (código presente; maestro Anexo 06 lista 8 prototipos base — usar numeración **A06-9/10** en Word si el asesor exige unicidad).

**Criterios.** UI español; datos ficticios; sin menores reales; coherencia con `frontend/src/App.tsx` y `navConfig.ts`.
...
## Anexo 09 — Guía de pantallas (8 figuras maestro + ampliación v2)

| Fig. | Ruta | Componente | Rol | RF |
| --- | --- | --- | --- | --- |
| 1 | `/login` | `LoginPage` | público | RF1 |
| 2 | `/app` | `AppShell` / `AppHomePage` | por rol | RF8 |
| 3 | `/app/circuito` | `CircuitPadrePage` | `PADRE` (+ admin) | RF3 |
| 4 | `/app/acceso/escaner` | `EscanerAccesoPage` | staff/docente | RF2 |
| 5 | `/app/modulos/calificaciones-docente` | `CalificacionesDocentePage` | `DOCENTE` | RF5 |
| 6 | `/app/modulos/finanzas` | `FinanzasPage` | padre/admin/alumno | RF6 |
| 7 | `/app/modulos/administracion` | `AdministracionPage` | administrativo | RF8 |
| 8 | (opc.) reportes SLA | API `admin-reports` + UI comunicación | administrativo | RF7 |
| 9* | `/app/modulos/comunicacion` | `ComunicacionPage` | todos | RF7 |
| 10* | `/app/institucion` | `InstitutionPage` | todos | RF4 |
| 11* | `/app/importaciones` | `ImportExportPage` | `ADMIN`, `ADMINISTRATIVO`, **`DOCENTE`** | RF4 |
| 12* | `/app/circuito/hoy` | `CircuitTodayPage` | docente/admin | RF3 |

**Prioridad:** capturas reales; datos sintéticos; pie del manual Anexo 09 del maestro.

#### Prompt IA (captura pantalla genérica)

```text
Captura de pantalla interfaz web Escuela Pass en español, tema claro Tailwind, ruta [RUTA], rol [ROL]. Datos ficticios, sin fotos de menores, sin marcas ajenas. Mostrar navegacion lateral coherente con navConfig. Resolucion legible impresion Word.
```
...
### Plantilla SUS (System Usability Scale) — 10 ítems en español

Escala Likert **1–5** (1 = totalmente en desacuerdo, 5 = totalmente de acuerdo). Calcular puntuación SUS estándar (0–100) según fórmula Brooke; citar Brooke (1996) y Lewis & Sauro (2018) en pie.

| # | Ítem (adaptado institucional) |
| --- | --- |
| 1 | Creo que usaría Escuela Pass con frecuencia en mi rol. |
| 2 | Encontré el sistema más complejo de lo necesario. *(invertido)* |
| 3 | Consideré que el sistema era fácil de usar. |
| 4 | Necesitaría apoyo técnico para operar el sistema. *(invertido)* |
| 5 | Las funciones están bien integradas en el producto. |
| 6 | Percibí inconsistencia entre pantallas. *(invertido)* |
| 7 | Imagino que la mayoría aprendería rápido a usarlo. |
| 8 | Me sentí torpe usando el sistema. *(invertido)* |
| 9 | Me sentí confiado al usar el sistema. |
| 10 | Necesité aprender mucho antes de poder operar. *(invertido)* |

**Método sugerido:** n ≥ 5 por rol institucional (administrativo, docente, padre) en entorno staging; sesión guiada 30–45 min; registrar en tabla Anexo 10.

### Plantilla ISO/IEC 25010 (características operativas)

| Flujo evaluado | Método | Entorno | n | Umbral | Resultado | Observaciones |
| --- | --- | --- | --- | --- | --- | --- |
| Login + refresh JWT | E2E + cronómetro | CI / staging | 47 specs | build verde | [rellenar] | `npm run test:e2e` |
| Circuito padre → entrega | E2E + UI | PostgreSQL 16 | 1 escenario | < 30 s API | [rellenar] | `circuit-requests` |
| Escaneo QR antiduplicación | E2E | staging | 1 | 409 en <10 s | [rellenar] | RF2 |
| Carga comprobante privado | Manual + API | staging | 3 | 403 sin token | [rellenar] | RF6 |
| Tablero administrativo | SUS parcial | producción demo | [n] | SUS ≥ 68 | [rellenar] | RF8 |
...
| Módulos dominio | 37 carpetas / 32 imports raíz | 37 dirs `src/modules/`; 32 en `AppModule` (+ `meetings` vía `DashboardModule`) |
...
**Mermaid cuerpo:** diff manual M1–M4 contra bloques ```mermaid del maestro v2 §3.1 (M1, Fig. 5–6) y §4.3–4.4 (M3, M4, Fig. 8–9).
...
## Matriz UC — Anexo 05 §2 × RF × código (67 filas)

Resumen de auditoría integrada en esta guía v2. Cada fila del maestro L1740–1827 tiene elipse en lámina **7-RF1…RF8** y evidencia en backend/frontend.

| Actor | Filas §2 | RF principales | % cobertura |
| --- | --- | --- | --- |
| Administrador plataforma | 11 | RF1, RF2, RF4, RF8 | 100 % |
| Personal administrativo | 18 | RF1, RF3–RF8 | 100 % |
| Docente | 13 | RF1–RF5, RF7–RF8 | 100 % |
| Padre o tutor | 16 | RF1, RF3–RF7 | 100 % |
| Alumno | 9 | RF1–RF3, RF5, RF7 | 100 % |
| **Total** | **67** | 8 láminas + índice | **100 %** |

**Mapeo RF × módulos × pantallas (extracto):**

| RF | Módulos backend | Pantallas clave |
| --- | --- | --- |
| RF1 | auth, privacy, audit | Login, Perfil, PrivacyGate |
| RF2 | access, class-attendance | EscanerAccesoPage |
| RF3 | circuit, vehicles, departure-consent | CircuitPadre, CircuitToday, CircuitDetail |
| RF4 | school, schools, uploads, settings, schedules, external-visits, meetings | Roster, Escuelas, Institución, Importaciones, Visitas, Reuniones |
| RF5 | attendance, activities, report-cards, attention-notes | Calificaciones, Boletines, Periodos, Anotaciones |
| RF6 | payments, files | FinanzasPage |
| RF7 | notices, notifications | ComunicacionPage, FCM |
| RF8 | dashboard, dashboards, reports, exports | AppHome, Administración |

**Veredicto Fig. 7:** láminas RF cubren **67/67** filas; sin elipses inventadas; matices documentales: comunicación (ruta sin nav), import docente (UI vs UC §2.2).

---

## Reevaluación v2 — calidad prompts (sustituye carpeta `_audit`)

| Figura / sección | Nivel v1 | Nivel v2 (`prompts_2`) | Cambio |
| --- | --- | --- | --- |
| Inventario § TDG | B | **A** | § 2.6/2.7, 3.1, 8 corregidos |
| Fig. 7 hub + RF1–8 | A | **A** | typo 18 filas §2.2; notas trazabilidad |
| Fig. 8–9 / M3–M4 | B | **A** | draw.io + prompts IA detallados |
| Anexo 05 seq. 4–5 | C | **A** | secuencias pago/actividad completas |
| Anexo 05 clases | C | **B** | torre Controller→Entity ampliada |
| Anexo 06 / 09 | B | **A** | rutas comunicación, institución, import |
| Anexo 10 SUS/ISO | C | **A** | plantillas ítems y tabla 25010 |
| QA final | C | **A** | §4.3–4.4; módulos 32/37 |

**Conclusión:** `docs/tdg/_audit/` puede **eliminarse**; la verificación numérica, matriz UC 67 filas y rating A/B/C viven en este archivo. Mantener solo `prompts-imagenes-ia.md` (histórico) y **`prompts_2.md`** (guía operativa).

**Scripts opcionales** (si se requiere regenerar CSV): re-ejecutar conteos con:

```powershell
(Get-ChildItem src/database/entities -Filter *.entity.ts).Count
(Get-ChildItem src/modules -Recurse -Filter *.controller.ts).Count
```
