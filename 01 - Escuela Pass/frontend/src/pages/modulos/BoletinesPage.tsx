  ...
  return (
    <div className="max-w-5xl animate-fade-in space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
        <label className="order-2 block w-full sm:order-1 sm:max-w-sm">
          <span className="sr-only">Buscar en la lista de boletines</span>
          <input
            type="search"
            value={bulletinSearch}
            onChange={(e) => setBulletinSearch(e.target.value)}
            placeholder="Buscar por alumno, matrícula, periodo…"
        ...
        <button
          ...
          onClick={() => void load()}
          disabled={loading}
          className="order-1 rounded border border-slate-300 px-3 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-60 sm:order-2"
        >
